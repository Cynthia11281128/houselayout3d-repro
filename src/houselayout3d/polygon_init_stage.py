...
class PolygonInitStageError(RuntimeError):
...
def _write_status(stage_dir: Path, state: str, detail: str = "") -> None:
    ...
    with (stage_dir / "polygon_init.log").open("a", encoding="utf-8") as log:
...
def fit_plane_ransac(
    ...
    if points.ndim != 2 or points.shape[1] != 3 or len(points) < 3:
        raise PolygonInitStageError("RANSAC needs at least three 3D points")
    if threshold <= 0 or iterations <= 0:
        raise PolygonInitStageError("RANSAC threshold and iterations must be positive")
    ...
    if best_mask is None or best_count < 3:
        raise PolygonInitStageError("RANSAC found no non-degenerate plane")
...
def group_boundary_contours(
    ...
    for index in range(len(loops)):
        ...
        while parent >= 0:
            if parent in seen:
                raise PolygonInitStageError("cyclic contour nesting detected")
            seen.add(parent)
            depths[index] += 1
...
def _verify_inputs(config: PipelineConfig, run_id: str) -> dict[str, Any]:
    ...
    manifest_path = run_dir / Stage.SKELETON.value / "manifest.json"
    if not manifest_path.is_file():
        raise PolygonInitStageError(f"06_skeleton manifest is missing: {manifest_path}")
    manifest = _read_json(manifest_path)
    if manifest.get("status") != "complete":
        raise PolygonInitStageError("06_skeleton manifest is not complete")
    ...
    records = {
        "structure_mesh": (Path(structure_record["path"]), structure_record["sha256"]),
        "semantic_mesh": (Path(full_record["path"]), full_record["sha256"]),
        "hard_labels": (Path(hard_record["path"]), hard_record["sha256"]),
        "label_names": (Path(labels_record["path"]), labels_record["sha256"]),
    }
    for name, (path, expected_hash) in records.items():
        if not path.is_file() or _sha256(path) != expected_hash:
            raise PolygonInitStageError(f"06_skeleton {name} hash no longer matches")
    segmentation = (
        run_dir
        / Stage.SKELETON.value
        / "spt"
        / f"level_{config.polygon_init.superpoint_level}_segmentation.npy"
    )
    if not segmentation.is_file():
        raise PolygonInitStageError(f"superpoint segmentation is missing: {segmentation}")
    return {
        "run_dir": run_dir,
        "manifest_path": manifest_path,
        "manifest": manifest,
        "structure_mesh": records["structure_mesh"][0],
        "semantic_mesh": records["semantic_mesh"][0],
        "hard_labels": records["hard_labels"][0],
        "label_names": records["label_names"][0],
...
def _mesh_artifact(path: Path, mesh: Any) -> dict[str, Any]:
    ...
    if not o3d.io.write_triangle_mesh(str(path), mesh, write_ascii=False):
        raise PolygonInitStageError(f"failed to write mesh: {path}")
    return {
        "path": str(path),
        "sha256": _sha256(path),
...
def run_polygon_init(
    ...
    stage_dir = inputs["run_dir"] / Stage.POLYGON_INIT.value
    if stage_dir.exists():
        raise PolygonInitStageError(
            f"polygon-init stage already exists and will not be overwritten: {stage_dir}"
        )
    stage_dir.mkdir(parents=True, exist_ok=False)
    started_at = datetime.now(timezone.utc)
    start = time.perf_counter()
    _write_status(stage_dir, "loading", "06_skeleton structure and superpoints")
    ...
    try:
        try:
            ...
            raise PolygonInitStageError("Open3D and SciPy are required") from error
        ...
        if len(structure_mesh.vertices) == 0 or len(structure_mesh.triangles) == 0:
            raise PolygonInitStageError("06_skeleton structure mesh is empty")
        ...
        if label_names != LAYOUT_LABELS:
            raise PolygonInitStageError("06_skeleton semantic label names changed")
        if len(hard_labels) != len(full_vertices) or len(segmentation) != len(full_vertices):
            raise PolygonInitStageError("full-mesh arrays do not match the semantic mesh")
        structure_to_full = np.flatnonzero(np.isin(hard_labels, [0, 1, 2, 3]))
        if len(structure_to_full) != len(vertices):
            raise PolygonInitStageError("structural label mask does not match structure mesh")
        if not np.allclose(vertices, full_vertices[structure_to_full], atol=1e-6):
            raise PolygonInitStageError("structure mesh vertex order is not the filtered full order")
        ...
        _write_status(
            stage_dir,
            "fitting_planes",
            f"vertices={len(vertices)} K={config.polygon_init.minimum_unassigned_vertices}",
        )
        candidate_index = 0
        while True:
            counts = np.bincount(
            ...
            if seed_count <= config.polygon_init.minimum_unassigned_vertices:
                break
            ...
            plane, seed_inliers = fit_plane_ransac(
                ...
                config.polygon_init.ransac_iterations,
                rng,
            ...
            global_inliers = np.flatnonzero(
                unassigned
                & (distances <= config.polygon_init.plane_distance_threshold_meters)
            )
            if len(global_inliers) == 0:
                raise PolygonInitStageError("RANSAC plane has no global inliers")
            ...
            if rejection is not None:
                ...
                for group in contour_groups:
                    ...
                    contour_masks = [
                        closed_rdp_mask(
                            points, config.polygon_init.rdp_epsilon_meters
                        )
        ...
        if not polygon_info:
            raise PolygonInitStageError("Algorithm 1 produced no valid polygons")
        _write_status(
            stage_dir,
            "writing_artifacts",
            f"polygons={len(polygon_info)} residual={int(unassigned.sum())}",
        )
        np.save(stage_dir / "structure_to_full_vertex.npy", structure_to_full)
        np.save(stage_dir / "assigned_plane_ids.npy", plane_assignments)
        np.save(stage_dir / "residual_unassigned_mask.npy", unassigned)
        _write_json(stage_dir / "polygon_info.json", polygon_info)
        with (stage_dir / "plane_candidates.jsonl").open("w", encoding="utf-8") as handle:
        ...
        rectified_record = _mesh_artifact(stage_dir / "clean_edge_mesh.ply", rectified_mesh)
        ...
        component_record = _mesh_artifact(
            stage_dir / "plane_components_mesh.ply", component_mesh
        )
        ...
        boundary_path = stage_dir / "polygon_boundaries.ply"
        if not o3d.io.write_line_set(str(boundary_path), line_set, write_ascii=False):
            raise PolygonInitStageError("failed to write polygon boundary line set")

        polygon_path = stage_dir / "polygon_info.json"
        array_names = (
        ...
        manifest = {
            ...
            "scene": config.scene,
            "run_id": run_id,
            "stage": Stage.POLYGON_INIT.value,
            "status": "complete",
            ...
            "algorithm": {
                ...
                "minimum_unassigned_vertices_K": config.polygon_init.minimum_unassigned_vertices,
                "plane_distance_threshold_meters": config.polygon_init.plane_distance_threshold_meters,
            ...
            "outputs": {
                ...
                "plane_candidates": {
                    "path": str(stage_dir / "plane_candidates.jsonl"),
                    "sha256": _sha256(stage_dir / "plane_candidates.jsonl"),
            ...
            "warnings": [
                ...
                "The supplied unofficial package consumes clean_edge_mesh.ply and polygon_info.json but does not contain the code that creates them; this stage independently implements the Appendix algorithm and its observed input schema.",
            ],
        }
        if not all(manifest["validation"].values()):
            raise PolygonInitStageError("polygon-init artifact validation failed")
        ...
        if isinstance(error, PolygonInitStageError):
            raise
        raise PolygonInitStageError(str(error)) from error
