"""Metric3D v2 inference for the active ``02_metric3d`` stage."""
...
class Metric3DStageError(RuntimeError):
...
def _verify_prior_stages(
    ...
    for required in (
        ...
        if not required.is_file():
            raise Metric3DStageError(f"required prior artifact is missing: {required}")
    ...
    if input_manifest.get("status") != "complete":
        raise Metric3DStageError("00_input manifest is not complete")
    if pose_manifest.get("status") != "complete":
        raise Metric3DStageError("01_pose manifest is not complete")
    if _sha256(image_list_path) != input_manifest["outputs"]["image_list_sha256"]:
        raise Metric3DStageError("00_input/images.txt hash no longer matches")
    if _sha256(transforms_path) != pose_manifest["outputs"]["transforms_json_sha256"]:
        raise Metric3DStageError("01_pose/transforms.json hash no longer matches")
    ...
    if len(image_names) != expected_count:
        raise Metric3DStageError("00_input image count no longer matches")
    if pose_manifest["validation"]["pose_count"] != expected_count:
        raise Metric3DStageError("01_pose count does not match 00_input")
    return input_manifest_path, pose_manifest_path, pose_manifest, image_names


def _git_revision(repository: Path) -> str:
    if not (repository / ".git").exists():
        raise Metric3DStageError(f"Metric3D repository is unavailable: {repository}")
...
def _load_model(config: PipelineConfig):
    ...
    if spec is None or spec.loader is None:
        raise Metric3DStageError(f"cannot load Metric3D hubconf: {hubconf_path}")
    ...
    if not isinstance(checkpoint, dict) or "model_state_dict" not in checkpoint:
        raise Metric3DStageError("unexpected Metric3D checkpoint structure")
...
def _run_one(config, model, torch, functional, np, cv2, image_path: Path):
    ...
    if "prediction_normal" not in output:
        raise Metric3DStageError("Metric3D v2 output has no prediction_normal")
    ...
    if not all(np.isfinite(array).all() for array in arrays):
        raise Metric3DStageError(f"Metric3D produced non-finite output: {image_path.name}")
    if depth_array.shape != (height, width) or normal_array.shape != (3, height, width):
        raise Metric3DStageError(f"Metric3D output dimensions are invalid: {image_path.name}")
...
def run_metric3d(
    ...
    input_manifest_path, pose_manifest_path, pose_manifest, image_names = (
        _verify_prior_stages(config, run_id)
    )
    weight_hash = _sha256(config.metric3d.weights)
    revision = _git_revision(config.metric3d.repository)

    run_dir = config.storage.outputs / config.scene / run_id
    stage_dir = run_dir / Stage.METRIC3D.value
    if stage_dir.exists():
        raise Metric3DStageError(
            f"Metric3D stage already exists and will not be overwritten: {stage_dir}"
        )
    depth_dir = stage_dir / "depth"
    geometry_dir = stage_dir / "geometry"
    ...
    _write_status(stage_dir, "loading_model", config.metric3d.model)
    ...
    try:
        ...
        _write_status(stage_dir, "running", f"0/{len(image_names)}")
        ...
        manifest = {
            ...
            "outputs": {
                ...
                "progress": str(stage_dir / "progress.json"),
            },
            "warnings": [
        ...
        if isinstance(error, Metric3DStageError):
            raise
        raise Metric3DStageError(str(error)) from error
