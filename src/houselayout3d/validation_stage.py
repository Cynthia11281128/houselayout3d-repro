"""Independent Stage11 validation for the exported HouseLayout3D layout."""
...
def next_attempt(stage_dir: Path) -> Path:
    indices = []
...
def require_hash(entry: Mapping[str, Any], label: str) -> Path:
    ...
    if not path.is_file():
        raise ValidationStageError(f"missing {label}: {path}")
    actual = sha256(path)
    if actual != entry["sha256"]:
        raise ValidationStageError(f"SHA256 mismatch for {label}: {path}")
    if path.stat().st_size != int(entry["size_bytes"]):
        raise ValidationStageError(f"size mismatch for {label}: {path}")
    return path
...
def mesh_report(path: Path, weld: bool = False) -> dict[str, Any]:
    ...
    if len(mesh.vertices) == 0 or len(mesh.triangles) == 0:
        raise ValidationStageError(f"empty mesh: {path}")
    if weld:
...
def triangle_xy_area(vertices: Iterable[Iterable[float]]) -> float:
    ...
    if points.shape != (3, 3):
        raise ValidationStageError(f"floor entity is not a 3D triangle: {points.shape}")
...
def validate_window(entity: Mapping[str, Any], minimum_points: int, minimum_size: float) -> dict[str, Any]:
    ...
    if vertices.shape != (4, 3) or not np.isfinite(vertices).all():
        raise ValidationStageError(f"invalid window vertices: {entity.get('entity_id')}")
    ...
    if closure > 1.0e-5 or coplanarity > 1.0e-5:
        raise ValidationStageError(f"window is not a planar parallelogram: {entity['entity_id']}")
    if int(entity["point_count"]) < minimum_points:
        raise ValidationStageError(f"window has too few points: {entity['entity_id']}")
    if float(entity["width_meters"]) <= minimum_size or float(entity["height_meters"]) <= minimum_size:
        raise ValidationStageError(f"window is below the paper size threshold: {entity['entity_id']}")
    return {
...
def run_validation(config: PipelineConfig, run_id: str) -> Path:
    ...
    stage_dir = run_dir / Stage.VALIDATION.value
    stage_dir.mkdir(parents=True, exist_ok=True)
    attempt = next_attempt(stage_dir)
    started_at = datetime.now(timezone.utc)
    started = time.perf_counter()
    write_status(stage_dir / "STATUS.json", "running", str(attempt))
    write_status(attempt / "STATUS.json", "loading", "Stage09 and Stage10 manifests")
    try:
        stage09_manifest_path = run_dir / Stage.SCENE_GRAPH.value / "manifest.json"
        stage10_manifest_path = run_dir / Stage.LAYOUT.value / "manifest.json"
        for path, stage in (
            (stage09_manifest_path, Stage.SCENE_GRAPH.value),
            (stage10_manifest_path, Stage.LAYOUT.value),
        ):
            if not path.is_file():
                raise ValidationStageError(f"missing completed manifest: {path}")
            payload = json.loads(path.read_text(encoding="utf-8"))
            if payload.get("stage") != stage or payload.get("status") != "complete":
                raise ValidationStageError(f"manifest is not complete for {stage}: {path}")
        stage09 = json.loads(stage09_manifest_path.read_text(encoding="utf-8"))
        stage10 = json.loads(stage10_manifest_path.read_text(encoding="utf-8"))
        write_status(attempt / "STATUS.json", "hashes", "checking Stage10 output records")
        output_paths = {
            name: require_hash(stage10["outputs"][name], f"Stage10 {name}")
            for name in (
                "layout_mesh",
                "layout_obj",
                "structures_mesh",
        ...
        for optional in ("windows_mesh", "doors_mesh", "stairs_mesh"):
            if optional in stage10["outputs"]:
                output_paths[optional] = require_hash(stage10["outputs"][optional], f"Stage10 {optional}")
        for category in ("rooms_closed", "rooms_final"):
            for room_id, entry in stage10["outputs"][category].items():
                require_hash(entry, f"{category} {room_id}")

        graph_path = Path(stage09["outputs"]["scene_graph"]["path"])
        ...
        if len(entity_ids) != len(set(entity_ids)):
            raise ValidationStageError("layout entity IDs are not unique")
        class_counts = Counter(str(value["class"]) for value in entities)
        if not set(class_counts).issubset(LAYOUT_CLASSES):
            raise ValidationStageError(f"unexpected entity classes: {set(class_counts) - LAYOUT_CLASSES}")
        if class_counts["floor"] == 0 or class_counts["ceiling"] == 0 or class_counts["wall"] == 0:
            raise ValidationStageError("structural entity class is empty")
        ...
        if invalid_room_references:
            raise ValidationStageError(f"invalid room references: {sorted(set(invalid_room_references))}")
        ...
        if max(area_errors.values(), default=0.0) > 1.0e-5:
            raise ValidationStageError(f"floor triangulation area mismatch: {area_errors}")
        ...
        closed_reports = {
            room_id: mesh_report(Path(stage10["outputs"]["rooms_closed"][room_id]["path"]), weld=True)
            for room_id in active_room_ids
        }
        final_reports = {
            room_id: mesh_report(Path(stage10["outputs"]["rooms_final"][room_id]["path"]), weld=True)
            for room_id in active_room_ids
        }
        for room_id, report in closed_reports.items():
            if not (report["edge_manifold_closed"] and report["vertex_manifold"] and report["orientable"]):
                raise ValidationStageError(f"closed room shell failed topology: {room_id}: {report}")
        graph_open_rooms = {
            str(room_id)
        ...
        for room_id in graph_open_rooms:
            if final_reports[room_id]["edge_manifold_closed"]:
                raise ValidationStageError(f"graph opening did not create a room boundary: {room_id}")
        ...
        if not all(final_graph_checks.values()):
            raise ValidationStageError(f"final scene graph failed: {final_graph_checks}")
        validation = {
        ...
        if not all(validation.values()):
            raise ValidationStageError(f"final validation failed: {validation}")
        ...
        report = {
            ...
            "known_limitations": [
                "Stage01 uses the supplied known poses rather than COLMAP poses, as explicitly selected for this run.",
                "Stage09 accepted no door or stair edges for this single-level scene; the final layout contains one ordinary opening.",
                "The paper does not disclose DBSCAN, LOF, voxel, and stair-step visualization settings; the explicit Stage10 YAML values were validated for reproducibility.",
        ...
        manifest = {
            ...
            "scene": config.scene,
            "run_id": run_id,
            "stage": Stage.VALIDATION.value,
            "status": "complete",
            "started_at": started_at.isoformat(),
            "finished_at": datetime.now(timezone.utc).isoformat(),
            "elapsed_seconds": round(time.perf_counter() - started, 6),
            "inputs": {
                "scene_graph_manifest": record(stage09_manifest_path),
                "layout_manifest": record(stage10_manifest_path),
            },
            "outputs": {
        ...
        write_status(stage_dir / "STATUS.json", "complete", str(manifest_path))
        ...
        if attempt.exists():
            ...
            write_status(stage_dir / "STATUS.json", "failed", str(failed_attempt))
        if isinstance(error, ValidationStageError):
            raise
        raise ValidationStageError(f"{type(error).__name__}: {error}") from error
