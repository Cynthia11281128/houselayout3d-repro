"""Immutable image input audit for stage ``00_input``."""
...
def prepare_input(
    ...
    """Audit configured images and write a new immutable ``00_input`` stage."""
    ...
    stage_dir = run_dir / Stage.INPUT.value
    if stage_dir.exists():
        raise InputAuditError(f"input stage already exists and will not be overwritten: {stage_dir}")
    stage_dir.mkdir(parents=True, exist_ok=False)

    image_list_path = stage_dir / "images.txt"
