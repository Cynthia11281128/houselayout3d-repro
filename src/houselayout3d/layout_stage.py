"""Paper-faithful Stage10 room extrusion and final layout generation."""
...
class LayoutStageError(RuntimeError):
    """Raised when Stage10 cannot satisfy the final-layout contract."""
...
def constrained_room_triangles(
    ...
    if not vertices or not segments:
        raise LayoutStageError("room constraint graph is empty")
    holes = []
    ...
    if not triangles:
        raise LayoutStageError("room constrained triangulation produced no triangles")
    return triangles
...
def detect_windows(
    ...
    if len(wall_tags) != len(wall_mesh.triangles):
        raise LayoutStageError("wall triangle instance tags are inconsistent")
    ...
    for frame_index, frame in enumerate(frames):
        ...
        if not semantic_path.is_file():
            raise LayoutStageError(f"missing COCO segmentation for window detection: {semantic_path}")
...
def run_layout(config: PipelineConfig, run_id: str) -> Path:
    """Run Stage10 and return its completed manifest path."""
    run_dir = config.storage.outputs / config.scene / run_id
    stage_dir = run_dir / Stage.LAYOUT.value
    stage_dir.mkdir(parents=True, exist_ok=True)
    attempt = _next_attempt(stage_dir)
    started_at = datetime.now(timezone.utc)
    started = time.perf_counter()
    _write_status(stage_dir / "STATUS.json", "running", str(attempt))
    _write_status(attempt / "STATUS.json", "loading_inputs", "Stage08/09/05/01")
    try:
        scene_graph_manifest_path = run_dir / Stage.SCENE_GRAPH.value / "manifest.json"
        scene_graph_manifest = _require_complete_manifest(
            scene_graph_manifest_path, Stage.SCENE_GRAPH.value
        )
        prototype_manifest_path = run_dir / Stage.PROTOTYPE.value / "manifest.json"
        prototype_manifest = _require_complete_manifest(
            prototype_manifest_path, Stage.PROTOTYPE.value
        )
        oneformer_manifest_path = run_dir / Stage.ONEFORMER.value / "manifest.json"
        oneformer_manifest = _require_complete_manifest(
            oneformer_manifest_path, Stage.ONEFORMER.value
        )
        pose_manifest_path = run_dir / Stage.POSE.value / "manifest.json"
        _require_complete_manifest(pose_manifest_path, Stage.POSE.value)
        ...
        transforms_path = run_dir / Stage.POSE.value / "transforms.json"
        coco_dir = Path(oneformer_manifest["outputs"]["coco_id_dir"])
        for path in (graph_path, rooms_path, state_path, transforms_path):
            if not path.is_file():
                raise LayoutStageError(f"required Stage10 input is missing: {path}")
        ...
        # Stage09 contours run through raster-cell centers. Expanding every room
        ...
        if not o3d.io.write_triangle_mesh(str(layout_obj_path), layout_mesh):
            raise LayoutStageError(f"failed to write OBJ: {layout_obj_path}")
        ...
        if not all(validation.values()):
            raise LayoutStageError(f"layout validation failed: {validation}")
        ...
        if isinstance(error, LayoutStageError):
            raise
        raise LayoutStageError(f"{type(error).__name__}: {error}") from error
