...
class SceneGraphError(RuntimeError):
    """Raised when scene_graph cannot satisfy its artifact contract."""
...
def _require_complete_manifest(path: Path, component: str) -> dict[str, Any]:
    if not path.is_file():
        raise SceneGraphError(f"missing {component} manifest: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("component") != component or payload.get("status") != "complete":
        raise SceneGraphError(f"{component} manifest is not complete: {path}")
    return payload
...
def load_prototype_data(state_path: Path, source_repository: Path) -> PrototypeData:
    ...
    if not np.isfinite(result.vertices).all():
        raise SceneGraphError("prototype contains non-finite vertices")
    if len(result.triangles) == 0 or len(result.polygon_classes) == 0:
        raise SceneGraphError("prototype geometry or semantic polygons are empty")
    return result
...
def semantic_polygon_ids(data: PrototypeData, name: str) -> list[int]:
    if name not in data.class_names:
        raise SceneGraphError(f"prototype has no semantic class {name!r}")
...
def multi_scale_room_watershed(
    ...
    if marker_count == 0:
        raise SceneGraphError("multi-scale watershed produced no room markers")
    ...
    if not room_rows:
        raise SceneGraphError("all watershed rooms failed the area threshold")
...
def run_scene_graph(config: PipelineConfig, run_id: str) -> Path:
    """Run Appendix D.1-D.5 on a completed prototype."""
    started_at = datetime.now(timezone.utc)
    started = time.perf_counter()
    run_root = config.storage.outputs / config.scene / run_id
    prototype_root = run_root / "prototype"
    ...
    component_dir = run_root / "scene_graph"
    if (component_dir / "manifest.json").is_file():
        raise SceneGraphError(
            f"scene-graph component already exists and will not be overwritten: {component_dir}"
        )
    ...
    try:
        ...
        skeleton_root = run_root / "skeleton"
        skeleton_manifest_path = skeleton_root / "manifest.json"
        _require_complete_manifest(skeleton_manifest_path, "skeleton")
        stair_mesh_path = skeleton_root / "stair_mesh.ply"
        transforms_path = run_root / "pose" / "transforms.json"
        ...
        if not floor_groups:
            raise SceneGraphError("prototype contains no floor levels")
        level_elevations = [
        ...
        for level_index, floor_group in enumerate(floor_groups):
            ...
            if footprint.is_empty:
                raise SceneGraphError(f"{level_id} footprint is empty")
            ceiling_elevation = (
        ...
        manifest = {
            ...
            "warnings": [
                ...
                "Prototype topology is not assumed watertight; scene_graph consumes its semantic planar polygons in BEV as described by Appendix D.",
            ],
        }
...
def main() -> int:
    ...
    return run_component(run_scene_graph, "Construct rooms and the scene graph.")
