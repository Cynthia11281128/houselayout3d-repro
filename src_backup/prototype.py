...
from .oneformer import LAYOUT_LABELS
...
def _verify_manifest_record(record: dict[str, Any], name: str) -> Path:
    ...
    if not path.is_file() or _sha256(path) != record["sha256"]:
        raise PrototypeError(f"prior artifact hash mismatch: {name}")
    return path


def _verify_inputs(config: PipelineConfig, run_id: str) -> dict[str, Any]:
    run_dir = config.storage.outputs / config.scene / run_id
    skeleton_manifest_path = run_dir / "skeleton" / "manifest.json"
    polygon_manifest_path = run_dir / "polygon_init" / "manifest.json"
    if not skeleton_manifest_path.is_file() or not polygon_manifest_path.is_file():
        raise PrototypeError("skeleton and polygon_init manifests are required")
    ...
    if skeleton.get("status") != "complete" or polygon.get("status") != "complete":
        raise PrototypeError("prior component is not complete")
...
def _simplify_object_mesh(
    ...
    if len(mesh.vertices) == 0 or len(mesh.triangles) == 0:
        raise PrototypeError("skeleton object mesh is empty")
    ...
    if len(mesh.vertices) == 0 or len(mesh.triangles) == 0:
        raise PrototypeError("object simplification produced an empty mesh")
...
def prepare_prototype(config: PipelineConfig, run_id: str) -> Path:
    """Freeze all prototype inputs and source/runtime audits before optimization."""
    ...
    component_dir = inputs["run_dir"] / "prototype"
    if component_dir.exists():
        raise PrototypeError(
            f"prototype component already exists and will not be overwritten: {component_dir}"
        )
    prepared_dir = component_dir / "prepared"
    ...
    try:
        ...
        manifest_path = component_dir / "prepared_manifest.json"
...
def _load_prepared(config: PipelineConfig, run_id: str) -> tuple[Path, dict[str, Any], dict[str, Path]]:
    component_dir = config.storage.outputs / config.scene / run_id / "prototype"
    manifest_path = component_dir / "prepared_manifest.json"
    if not manifest_path.is_file():
        raise PrototypeError("prepare-prototype must complete first")
    manifest = _read_json(manifest_path)
    if manifest.get("status") != "prepared":
        raise PrototypeError("prepared prototype manifest is invalid")
    if (component_dir / "manifest.json").is_file():
        complete = _read_json(component_dir / "manifest.json")
...
def _next_attempt(component_dir: Path) -> Path:
    indices = []
    for path in component_dir.glob("attempt_*_*"):
        match = re.match(r"attempt_(\d+)_", path.name)
        if match:
            indices.append(int(match.group(1)))
    index = max(indices, default=0) + 1
    return component_dir / f"attempt_{index:03d}_running"
...
def fit_prototype(config: PipelineConfig, run_id: str) -> Path:
    ...
    component_dir, prepared_manifest, frozen = _load_prepared(config, run_id)
    ...
    if [record["step"] for record in state_snapshots] != expected_steps:
        raise PrototypeError("model-state checkpoint steps are incomplete")
    ...
    manifest = {
        ...
        "inputs": {
            "prepared_manifest": _record(component_dir / "prepared_manifest.json"),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Prepare or fit the prototype component."
    )
    ...
    if args.command == "prepare":
        path = prepare_prototype(config, args.run_id)
    else:
        path = fit_prototype(config, args.run_id)
    print(path)
    return 0
