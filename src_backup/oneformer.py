...
def _verify_inputs(
    ...
    mesh_manifest_path = run_dir / "mesh" / "manifest.json"
    if not input_manifest_path.is_file() or not mesh_manifest_path.is_file():
        raise OneFormerError("completed input and mesh manifests are required")
    ...
    if input_manifest.get("status") != "complete":
        raise OneFormerError("input manifest is not complete")
    ...
    if not image_list.is_file():
        raise OneFormerError(f"frozen image list is missing: {image_list}")
    if _sha256(image_list) != input_manifest["outputs"]["image_list_sha256"]:
        raise OneFormerError("input image-list hash no longer matches")
    ...
    if names != [record.get("name") for record in records]:
        raise OneFormerError("input image records no longer match images.txt")
    ...
    if not mesh_path.is_file() or _sha256(mesh_path) != mesh_record["sha256"]:
        raise OneFormerError("mesh Poisson artifact hash no longer matches")
...
def _label_contract(id2label: dict[int, str]) -> dict[str, Any]:
    if set(id2label) != set(range(133)):
        raise OneFormerError("OneFormer checkpoint does not expose COCO's 133 IDs")
    expected = {
        85: "curtain",
        86: "door-stuff",
        87: "floor-wood",
    ...
    if mismatches:
        raise OneFormerError(f"COCO label contract mismatch: {mismatches}")
...
def run_oneformer(
    ...
    component_dir = (
        config.storage.outputs / config.scene / run_id / "oneformer"
    )
    if component_dir.exists():
        raise OneFormerError(
            f"OneFormer component already exists and will not be overwritten: {component_dir}"
        )
    coco_dir = component_dir / "coco_id"
    layout_dir = component_dir / "layout_id"
    preview_dir = component_dir / "previews"
    coco_dir.mkdir(parents=True, exist_ok=False)
    layout_dir.mkdir()
    preview_dir.mkdir()
    _write_status(component_dir, "loading_model", str(config.oneformer.model_dir))
    ...
    if not torch.cuda.is_available():
        _write_status(component_dir, "failed", "CUDA is unavailable")
        raise OneFormerError("OneFormer component requires CUDA")
    ...
    log_path = component_dir / "inference.log"
    records_path = component_dir / "per_image.jsonl"
    labels_path = component_dir / "labels.json"
    try:
        ...
        _write_status(component_dir, "inferencing", f"0/{count}")
        with log_path.open("w", encoding="utf-8") as log, records_path.open(
            ...
            for index, source_record in enumerate(image_records):
                ...
                if not image_path.is_file():
                    raise OneFormerError(f"input image is missing: {image_path}")
                ...
                if int(coco.max()) >= 133:
                    raise OneFormerError("OneFormer output contains a non-COCO ID")
                layout = lut[coco]
                ...
                if (index + 1) % 10 == 0 or index + 1 == count:
                    _write_status(component_dir, "inferencing", progress)
...
def main() -> int:
    ...
    return run_component(run_oneformer, "Run OneFormer semantic inference.")
