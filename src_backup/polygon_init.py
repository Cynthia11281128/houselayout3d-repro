...
def _verify_inputs(config: PipelineConfig, run_id: str) -> dict[str, Any]:
    ...
    manifest_path = run_dir / "skeleton" / "manifest.json"
    if not manifest_path.is_file():
        raise PolygonInitError(f"skeleton manifest is missing: {manifest_path}")
    ...
    for name, (path, expected_hash) in records.items():
        if not path.is_file() or _sha256(path) != expected_hash:
            raise PolygonInitError(f"skeleton {name} hash no longer matches")
    segmentation = (
        run_dir
        / "skeleton"
        / "spt"
...
def run_polygon_init(
    ...
    component_dir = inputs["run_dir"] / "polygon_init"
    ...
    try:
        ...
        rectified_record = _mesh_artifact(component_dir / "clean_edge_mesh.ply", rectified_mesh)
...
def main() -> int:
    ...
    return run_component(run_polygon_init, "Initialize structural polygons.")
