"""Known-pose adapter for the active ``pose`` component."""
...
def _verify_input_component(
    config: PipelineConfig, run_id: str
) -> tuple[Path, dict[str, Any], list[str]]:
    input_dir = config.storage.outputs / config.scene / run_id / "input"
    manifest_path = input_dir / "manifest.json"
    image_list_path = input_dir / "images.txt"
    if not manifest_path.is_file() or not image_list_path.is_file():
        raise PoseError(f"complete input component is missing: {input_dir}")
    manifest = _read_json(manifest_path)
    if manifest.get("status") != "complete":
        raise PoseError("input manifest is not complete")
    if _sha256(image_list_path) != manifest["outputs"]["image_list_sha256"]:
        raise PoseError("input/images.txt hash no longer matches its manifest")
    names = image_list_path.read_text(encoding="utf-8").splitlines()
    if len(names) != manifest["validation"]["image_count"]:
        raise PoseError("input image count no longer matches images.txt")
...
def _quaternion_to_matrix(values: tuple[float, float, float, float]) -> list[list[float]]:
    ...
    if norm == 0.0:
        raise PoseError("pose contains a zero-norm quaternion")
...
def _read_pose_rows(path: Path, expected_names: list[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if not path.is_file():
        raise PoseError(f"configured pose CSV does not exist: {path}")
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        try:
            ...
            raise PoseError(f"pose CSV is empty: {path}") from error
        ...
        for line_number, raw in enumerate(reader, start=2):
            ...
            if not all(math.isfinite(value) for value in numeric):
                raise PoseError(f"pose CSV row {line_number} is not finite")
...
def prepare_poses(
    ...
    if config.input.poses_csv is None or config.input.pose_convention is None:
        raise PoseError("known poses are not configured")
    if config.input.pose_convention != "opencv_c2w_xyzw_meters":
        raise PoseError(f"unsupported pose convention: {config.input.pose_convention}")

    started_at = datetime.now(timezone.utc)
    start = time.perf_counter()
    input_dir, input_manifest, image_names = _verify_input_component(config, run_id)
    rows, pose_stats = _read_pose_rows(config.input.poses_csv, image_names)

    component_dir = config.storage.outputs / config.scene / run_id / "pose"
    if component_dir.exists():
        raise PoseError(
            f"pose component already exists and will not be overwritten: {component_dir}"
        )
    component_dir.mkdir(parents=True, exist_ok=False)
    images_link = component_dir / "images"
    ...
    transforms_path = component_dir / "transforms.json"
    ...
    manifest = {
        ...
        "outputs": {
            "dataset_root": str(component_dir),
...
def main() -> int:
    ...
    return run_component(prepare_poses, "Validate known poses and write transforms.")
